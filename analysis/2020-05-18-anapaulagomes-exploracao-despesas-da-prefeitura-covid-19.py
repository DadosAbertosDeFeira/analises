...
pagamentos = despesas_com_classificacao[
    despesas_com_classificacao["phase"] == "PAGAMENTO"
]

plt.figure(figsize=(16, 8))
plot = sns.barplot(
    x=pagamentos["Descricao"], y=pagamentos.value, estimator=sum, log=False, ci=None,
)
...
pagamentos.groupby(["company_or_person"])["value"].sum().to_frame()


# In[97]:


pagamentos.groupby(["Descricao"])["value"].sum().to_frame()
